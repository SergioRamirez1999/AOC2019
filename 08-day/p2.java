...
public class p1 {
	...
	private static List<String> decode_image(List<List<String>> layers) {
		List<String> decode_image = new ArrayList<>();
		List<String> first_layer = layers.get(0);

		for (int i = 0; i < first_layer.size(); i++) {
			String row_decoded = "";
			int row_index = i;
			char[] row = first_layer.get(i).toCharArray();
			for(int h=0; h<row.length; h++) {
				row_decoded = row_decoded.concat(String.valueOf(determine_color(layers, 0, row_index, h)));
			}
			decode_image.add(row_decoded);
		}
		
		return decode_image;
	}
	
	private static void show_message(List<String> decoded_image) {
		for(String row: decoded_image) {
	...
	public static void main(String[] args) {
		...
		List<String> decoded_image = decode_image(layers);
		
		show_message(decoded_image);
